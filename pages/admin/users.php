...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_user'])) {
        ...
        // Validate inputs
        if (empty($email) || empty($first_name) || empty($last_name) || empty($role) || empty($password)) {
            $error = "All fields are required.";
        } elseif (!validate_email($email)) {
            $error = "Invalid email format.";
        } else {
            // Check if email already exists
            $check_sql = "SELECT id FROM users WHERE email = '" . mysqli_real_escape_string($conn, $email) . "'";
            ...
            if (mysqli_num_rows($check_result) > 0) {
                ...
                $insert_sql = "INSERT INTO users (email, password, first_name, last_name, role) 
        ...
        if (empty($email) || empty($first_name) || empty($last_name) || empty($role)) {
            ...
            $check_sql = "SELECT id FROM users WHERE email = '" . mysqli_real_escape_string($conn, $email) . "' AND id != $user_id";
            $check_result = mysqli_query($conn, $check_sql);
...
if (isset($_GET['quick_action'])) {
    ...
    switch ($quick_action) {
        ...
        case 'delete':
            // Soft delete - we don't actually delete the user
            $sql = "UPDATE users SET status = 'inactive' WHERE id = $user_id";
            if (mysqli_query($conn, $sql)) {
...
if (!empty($role_filter)) {
    $where_conditions[] = "role = '" . mysqli_real_escape_string($conn, $role_filter) . "'";
}
...
// Get users
$users_sql = "SELECT * FROM users $where_clause ORDER BY created_at DESC LIMIT $offset, $records_per_page";
